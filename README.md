...
To declare all types internal, just declare the class as internal:
```cs
using AvroSourceGenerator;
...
Example for making generated code compatible with C# 7.3 (.NET Framework and .NET Standard 2.0):
```cs
using AvroSourceGenerator;
...
It is also possible to disable specific features.

Example for using latest features expected required propertyies:
```cs
using AvroSourceGenerator;

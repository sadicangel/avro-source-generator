...
internal static class RegisterSchemaUnionExtensions
{
    extension(ref SchemaRegistry schemaRegistry)
    {
        public UnionSchema Union(JsonElement schema, string? containingNamespace)
        {
            ...
            var hasQuestionMark = isNullable && (schemaRegistry.Options.UseNullableReferenceTypes || MapsToValueType(underlyingSchema.Type));

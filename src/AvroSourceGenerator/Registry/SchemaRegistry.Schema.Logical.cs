...
internal readonly partial struct SchemaRegistry
{
    private AvroSchema Logical(JsonElement schema, string? containingNamespace, ImmutableSortedDictionary<string, JsonElement>? properties = null)
    {
        ...
        static bool TryGetNamedSchema(Dictionary<SchemaName, TopLevelSchema> schemas, string underlyingType, string? containingNamespace, [MaybeNullWhen(false)] out NamedSchema namedSchema)
        {
            var schemaName = new SchemaName(JsonElementExtensions.GetValidName(underlyingType), containingNamespace);
            namedSchema = schemas.TryGetValue(schemaName, out var topLevelSchema) ? topLevelSchema as NamedSchema : null;
...
{
    public static AvroSchema Create(string logicalType, AvroSchema underlyingSchema, ImmutableSortedDictionary<string, JsonElement> properties) => logicalType switch
    {
        "date" => new LogicalSchema(underlyingSchema, new CSharpName("DateTime", "System"), new SchemaName(logicalType), properties),
        "decimal" => new LogicalSchema(underlyingSchema, new CSharpName("decimal"), new SchemaName(logicalType), properties),
        "duration" => new LogicalSchema(underlyingSchema, underlyingSchema.CSharpName, new SchemaName(logicalType), properties),
        "time-micros" => new LogicalSchema(underlyingSchema, new CSharpName("TimeTime", "System"), new SchemaName(logicalType), properties),
        "time-millis" => new LogicalSchema(underlyingSchema, new CSharpName("TimeTime", "System"), new SchemaName(logicalType), properties),
...
file static class LogicalNoneCSharp10OrLater
{
    public static AvroSchema Create(string logicalType, AvroSchema underlyingSchema, ImmutableSortedDictionary<string, JsonElement> properties) => logicalType switch
    {
        "date" => new LogicalSchema(underlyingSchema, new CSharpName("DateOnly", "System"), new SchemaName(logicalType), properties),
        "decimal" => new LogicalSchema(underlyingSchema, new CSharpName("decimal"), new SchemaName(logicalType), properties),
        "duration" => new LogicalSchema(underlyingSchema, underlyingSchema.CSharpName, new SchemaName(logicalType), properties),
        "time-micros" => new LogicalSchema(underlyingSchema, new CSharpName("TimeOnly", "System"), new SchemaName(logicalType), properties),
        "time-millis" => new LogicalSchema(underlyingSchema, new CSharpName("TimeOnly", "System"), new SchemaName(logicalType), properties),
        "timestamp-micros" => new LogicalSchema(underlyingSchema, new CSharpName("DateTime", "System"), new SchemaName(logicalType), properties),
        "timestamp-millis" => new LogicalSchema(underlyingSchema, new CSharpName("DateTime", "System"), new SchemaName(logicalType), properties),
        "local-timestamp-micros" => new LogicalSchema(underlyingSchema, new CSharpName("DateTime", "System"), new SchemaName(logicalType), properties),
        "local-timestamp-millis" => new LogicalSchema(underlyingSchema, new CSharpName("DateTime", "System"), new SchemaName(logicalType), properties),
